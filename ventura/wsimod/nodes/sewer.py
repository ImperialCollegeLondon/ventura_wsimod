...
@author: bdobson
"""
from ventura.wsimod.nodes.nodes import Node, QueueTank
...
class EnfieldFoulSewer(Sewer):
    ...
    def make_discharge(self):
        ...
        if self.sewer_tank.storage['volume'] > self.storm_exchange * self.sewer_tank.capacity:
            ...
            #vk tried to change the above to the below for otladka as was causing problems
            remaining = self.push_distributed(exchange, 'Sewer')
        ...
        remaining = self.push_distributed(self.sewer_tank.active_storage,
                                        of_type = ['Sewer','Waste',"WWTW"],
                                        tag='Sewer')
        #VK changed the of_type = ['Waste'] to the current value on Barney's advise in Jul 2022
